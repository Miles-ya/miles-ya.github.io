---
title: 蓝桥赛前小记 往期视频总结
date: '2025-04-07T20:25:32+08:00'
aliases:
  - '/posts/2025/11/蓝桥赛前小记-往期视频总结/'
draft: false
tags:
  - 课程笔记
  - 算法
  - 蓝桥杯
category: 资源
description: 总结了蓝桥杯赛前注意事项和各类算法题单链接。
updated: '2025-08-03 16:35:22'
created: '2025-04-07'
---
...
二分题单: https://www.luogu.com.cn/training/111#problems

双指针题单: https://rentry.org/d4y8o2dn

DFS题单: https://rentry.org/5yhk52ow

BFS题单: https://rentry.org/3r68rga7

动态规划入门课题单: https://rentry.org/2f76axt4

动态规划习题课题单:  https://rentry.org/ozd34yrn

赛前小记: https://rentry.org/5p2wa89m
### 首先是蓝桥杯的注意事项：
...
6. `int`  型变量最大记为 **2e9**  , `long long` 最大记为 **1e18** （其实是9e18，主要记数量级），OI 赛制 **不开long long见祖宗！**
7. 输入输出建议直接使用 `scanf` 和 `printf` ，或者ios加速后的 `cin` 和 `cout` ，但**切记**使用加速命令后`cin` `cout` 不要同时和 `scanf` 和 `printf` 一起使用。

![图片描述](https://dn-simplecloud.shiyanlou.com/questions/uid1736686-20240410-1712735485378) 直接放入 `main()` 函数开头即可。

8. 输出的时候如果两个数字间有空格的话记得输出需要 ``` printf("%d %d\n", a, b);``` （往期很多同学都会犯）
9. 大家如果平时不常用 debug 功能的话，可以去在关键处使用 **printf 大法** 去打印出**关键变量（或者数组中的元素）**，思考一下写的代码和思路的结果是否相同。
10. 题目的样例给的很少，可以自己去**造几组数据**进行测试（最好**造边界、极端情况** ），考虑有没有负值、有没有特殊样例、有没有可能爆变量的最大值。
11. 代码全都写进主函数有点难以调试，可以去 **分块写** 函数，哪里错了看哪里

12. 最好熟悉使用一下 **stl** 的数据结构 string , map, set,queue, stack, priority_queue, vector等；同时最好也熟悉 stl 里的函数
...
16. **数组大小要开的大一点**，比如题干给 $n \le 10000$ ，咱们最好开 $10010$ 大的数组，防止越界。
17. **注意二分 $l, r$ 的取值**，如果数组从下标为 $1$ 处开始存，则 $l = 0, r = n + 1$ ；如果从下标为 $0$ 处开始存，则 $l = -1, r = n$ 即可。
18. 要清楚**边界线**，以及怎么去划定自己需要的范围。
...
19. 要保证两个指针的**单调性**，即**不能回头走** （如果向前走就一直向前走）。
20. 如果无序的话，可能需要先**排个序**。
21. 用到的基本是 **快慢指针** 和 **对撞指针**。
...
22. 注意枚举的顺序，要做到对每个点都依次枚举到。
23. 注意输出方案时 $return$ 的位置。
24. 要从数据存入的第一个点开始依次搜（比如数组从 $1$ 开始存，那么 $dfs(1)$ ）。
25. 迷宫问题的方向数组别写错
26. $dfs$​ 前可以预处理
